...
{   
    ...
    scanf("%s",venue);
    fprintf(fp, "Venue : %s", venue);

    printf("\nEnter First team's name\n");
    scanf("%s",team1);

    printf("\nEnter Second team's name\n");
    scanf("%s",team2);
    ...
    if(strcmp(toss, team1)==0)
    {
  	strcpy(ntoss,team2);
	}
    else {strcpy(ntoss,team1);}
    ...
    fprintf(fp,"Runs scored by team %s are %.0f in %.1f overs\n",toss,total_scr, total_ovs);
    ...
    printf("The runs scored by team %s are %.0f in %.1f overs\n", ntoss, total_scr1, total_ovs1);
    fprintf(fp, "The runs scored by team %s are %.0f in %.1f overs\n", ntoss, total_scr1, total_ovs1);
        ...
        case 2 : fp = fopen("Score.txt","r");
                 if (fp==NULL)
               {
                 ...
                 return 1;
                   ...
                   printf("Check Score.txt is opened...");
                 }
               fclose(fp); 
               break;
