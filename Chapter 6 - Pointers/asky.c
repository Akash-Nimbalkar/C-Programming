...
{   
    ...
    printf("\t\t\t\t\t\t** Welcome to cricket Match **\n \n");
    ...
    printf("\n\t\t\tWelcome to %s Vs %s %d overs match...\n",team1,team2, k);
    printf("\t\t\t    ..%s %s At %s %s..\n\n", time, ap, venue, date);
    ...
    printf("\n");
    fprintf(fp,"\t\t\t\t\tTeam %s's batting scorecard !!", toss);
    ...
    for(i=0; i<5; i++)
    {
        ...
        scanf("%s %f %f %f", Bw[i].name, &Bw[i].overs, &Bw[i].runs, &Bw[i].wickets);
     	...
     	float total_runs=0, total_runs1=0;
     	float total_overs, total_overs1;
    ...
    {   
        ...
        fprintf(fp,"%s\t\t\t%.0f\t%.0f\t%.2f\n", Bw[i].name, &Bw[i].overs, &Bw[i].runs, Bw[i].wickets, economy);
        total_runs = total_runs + Bw[i].runs;
        total_overs = total_overs + Bw[i].overs;
    }
    ...
    printf("The runs scored by %s team is %.0f in %.1f overs\n", ntoss, total_runs1, total_overs1);
    printf(fp,"The runs scored by %s team is %.0f in %.1f overs\n", &ntoss, &total_runs1, &total_overs1);
